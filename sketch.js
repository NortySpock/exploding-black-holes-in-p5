...
background(whiteSpaceFill);
        ...
        for (var i = 0; i < array_width; i++) {
            ...
            if (isDead) {
                ...
                radius = radius_array[i] = radius_array[i] + 10;
            }

            if (xpos > 0 && ypos > 0 && radius > 0) {
                numberOfBlackHoles++;
                // if we hit a circle
                if (!isDead && mousePressed == true && mouseButton == LEFT
                        && dist(mouseX, mouseY, xpos, ypos) <= radius) {
                    fill(whiteSpaceFill);
        ...
        if (isDead) {
            ...
            text("Zapped:" + BlackHolesExploded, canvasWidth / 2, canvasHeight * 3
                    / 4);
        }
